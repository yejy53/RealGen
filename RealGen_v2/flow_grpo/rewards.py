...
def register_external_model(name, model):
    global EXTERNAL_MODELS
    EXTERNAL_MODELS[name] = model


def setup_trainable_scorers(reward_fn, device, dtype=torch.float32):
    """
    Scans reward_fn. If a key is in SUPPORTED_TRAINABLE_KEYS,
    it initializes the TrainableScorer automatically.
    """
    from flow_grpo.trainable_scorers import TrainableScorer, train_discriminator_step

    active_scorers = {}
    train_step_fn = train_discriminator_step
    
    for name in reward_fn.keys():
        if name in SUPPORTED_TRAINABLE_KEYS:
            logger.info(f"[Rewards] Auto-initializing trainable scorer: '{name}'")
            # Initialize specific scorer
            scorer = TrainableScorer(model_name=name, device=device, dtype=dtype)
            # Register to global dict so reward functions can find it
            register_external_model(name, scorer)
            # Add to active list
            active_scorers[name] = scorer
            
    return active_scorers, train_step_fn
...
def omniaid_score(device):
    ...
    def _fn(images, prompts, metadata):
        scorer = EXTERNAL_MODELS.get("omniaid")     
        if scorer is None: 
            return torch.zeros(len(images), device=device), {} 
        if isinstance(images, torch.Tensor):
            ...
            if images.ndim == 4: # Batch Images
                images = [Image.fromarray(image) for image in images]
            elif images.ndim == 5: # Batch Videos
                images = [image for image in images]
            
        return scorer.get_reward(images), {}
    
    return _fn


def omniaid_dino_score(device):

    def _fn(images, prompts, metadata):
        scorer = EXTERNAL_MODELS.get("omniaid-dino")     
        if scorer is None: 
            return torch.zeros(len(images), device=device), {} 
        if isinstance(images, torch.Tensor):
            if images.ndim == 4 and images.shape[1] == 3: 
...
def longclip_score(device):
    from flow_grpo.longclip_scorer import LongClipScorer

    scorer = LongClipScorer(device=device)
...
def visualquality_score(device):
    from flow_grpo.visualquality_scorer import VisualQualityScorer

    scorer = VisualQualityScorer(device=device, dtype=torch.bfloat16)
...
def qwenfake_score(device):
    from flow_grpo.qwenfake_scorer import QwenVLScorer
...
def multi_score(device, score_dict):
    score_functions = {
        ...
        "omniaid-dino": omniaid_dino_score,
        "qwenfake": qwenfake_score,
        "longclip": longclip_score,
        "visualquality": visualquality_score,
    }
    score_fns={}
