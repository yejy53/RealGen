...
from flow_grpo.Long_CLIP.model import longclip
    

class LongClipScorer(torch.nn.Module):
    def __init__(self, device):
        super().__init__()
        self.device = device
        self.model, self.preprocess = longclip.load("LongCLIP-L/longclip-L.pt", device=self.device)
        self.model.eval()
...
def main():
    scorer = LongClipScorer(device='cuda')
