...
class EffortScorer(torch.nn.Module):
    def __init__(self, device="cuda", dtype=torch.bfloat16):
        ...
        checkpoint = torch.load('../RealGen/models/checkpoint-0.pth', map_location='cpu')
...
def main():
    ...
    img_path = ['test_image0.png']
    for img in img_path:
        pil_images = [Image.open(f'{img}').convert('RGB')]
        print(img[:-4], f"{scorer(pil_images)[0]:.8f}")
