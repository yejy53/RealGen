...
def calculate_zero_std_ratio(prompts, gathered_rewards):
    ...
    grouped_rewards = gathered_rewards['avg'][np.argsort(inverse_indices)]
...
def multisocre_countforwandb(reward_type, rewards_dict):
    ...
    rewards_string = " | ".join(reward_parts)
            ...
            wandb.log(
                {
                    "eval_images": [
                        wandb.Image(
                            ...
                            caption=f"{prompt:.100} | " + " | ".join(f"{k}: {v:.2f}" for k, v in reward.items() if v != -10),
                        )
...
def main(_):
    ...
    if config.prompt_fn == "realgen":
        ...
        raise NotImplementedError("Only realgen is supported with dataset")
    ...
    while True:
            ...
            samples.append(
                {
                    ...
                    "image_ids": image_ids.unsqueeze(0).repeat(config.sample.mini_num_image_per_prompt*len(prompt_ids),1,1),
                    "timesteps": timesteps,
                    "latents": latents[
                        :, :-1
        ...
        if epoch % 10 == 0 and accelerator.is_main_process:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                reward_type = list(config.reward_fn.keys()) + ['avg']
                sampled_rewards = [tuple(rewards[key][i] for key in reward_type) for i in sample_indices]
                ...
                wandb.log(
                    {
                        "images": [
                            wandb.Image(
                                os.path.join(tmpdir, f"{idx}.jpg"),
                                caption=f"{prompt:.100} | {multisocre_countforwandb(reward_type, avg_reward)}",
                            )
        ...
        if config.per_prompt_stat_tracking:
            ...
            group_size, trained_prompt_num = stat_tracker[config.reward_fn.keys()[0]].get_stats() # qwenvlfake
